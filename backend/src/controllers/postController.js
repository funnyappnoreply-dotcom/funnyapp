...
const feed = async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ error: 'Erro ao carregar feed.' }) }
}
...
const criar = async (req, res) => {
  try {
    ...
    const isVideo = tiposVideo.includes(arquivo.mimetype)

    const resultado = await uploadParaCloudinary(arquivo.buffer, arquivo.mimetype, arquivo.originalname)
...
const buscarPorId = async (req, res) => {
  try {
    ...
    res.json({ post })
  } catch (e) { res.status(500).json({ error: 'Erro ao buscar post.' }) }
}
...
const curtir = async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ error: 'Erro ao curtir.' }) }
}
...
const deletarComentario = async (req, res) => {
  ...
  } catch (e) { res.status(500).json({ error: 'Erro ao deletar comentário.' }) }
}
...
const postsPorUsuario = async (req, res) => {
  try {
    ...
    const posts = await Post.find({ autor: usuario._id })
      ...
      .sort({ createdAt: -1 })
    res.json({ posts })
  } catch (e) { res.status(500).json({ error: 'Erro ao buscar posts.' }) }
}
