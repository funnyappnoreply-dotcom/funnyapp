...
const dashboard = async (req, res) => {
  try {
    ...
    const postsRecentes = await Post.find()
      .populate('autor', 'username avatar perfil')
      .sort({ createdAt: -1 })
      .limit(5)

    const usuariosRecentes = await Usuario.find()
      .sort({ createdAt: -1 })
      .limit(5)
...
// PUT /api/admin/usuarios/:id/perfil — dar/tirar admin
const alterarPerfil = async (req, res) => {
  try {
    ...
    // Superadmin não pode ser rebaixado por admin comum
    ...
    // Só superadmin pode promover para superadmin
...
// PUT /api/admin/usuarios/:id/banir
...
// PUT /api/admin/usuarios/:id/desbanir
...
const listarPosts = async (req, res) => {
  try {
    ...
    const posts = await Post.find(filtro)
      .populate('autor', 'username avatar perfil banido')
      .sort({ createdAt: -1 })
      .skip((pagina - 1) * limite)
      .limit(Number(limite))
...
// DELETE /api/admin/posts/:id — admin pode deletar qualquer post
...
const listarDenuncias = async (req, res) => {
  try {
    ...
    const denuncias = await Denuncia.find(filtro)
      .populate('post', 'imagem legenda autor')
      .populate('denunciante', 'username avatar')
      .populate('analisadaPor', 'username')
      .sort({ createdAt: -1 })
      .skip((pagina - 1) * limite)
      .limit(Number(limite))
...
const analisarDenuncia = async (req, res) => {
  try {
    ...
    // Se admin decidir deletar o post junto
    ...
    res.status(500).json({ error: 'Erro ao analisar denúncia.' })
  }
}

module.exports = { dashboard, listarUsuarios, alterarPerfil, banirUsuario, desbanirUsuario, listarPosts, deletarPost, listarDenuncias, analisarDenuncia }
